...
class TrainingConfig:
    ...
    '''
    DATASET = ''                # Dataset name (mnist_unet_diffusion, CIFAR, Imagenet, CelebA)
...
def sample_diffusion_from_noise(model, n_images=25, config=TrainingConfig(), df=DiffusionConfig(), dim=3, snap_steps=None):
    ...
    model.eval()
    snap_set = set(snap_steps) if snap_steps is not None else set()
    history = {}
    ...
    for t in reversed(range(0, config.TIMESTEPS)):
        ...
        x = (one_by_sqrt_alpha_t * (x - (beta_t / sqrt_one_minus_alpha_cumulative_t) * eps_ts) + torch.sqrt(beta_t) * z)
        
        if t in snap_set:
            history[t] = x.cpu().clone()
            
    if snap_steps is not None:
        return x, x_init, history
    return x, x_init, None
...
def train(model, trainloader, optimizer, config, sd, loss_fn, sweep=1., save_every=500, suffix='', data_snaps_steps=False):
    ...
    if data_snaps_steps:
        snap_data_steps = list(range(0, config.N_STEPS, 10))
    else:
        snap_data_steps = None
